...
import {
  C2E_DIGITAL_DOCUMENT_TYPE,
  C2E_RESOURCE_COLLECTION_ID,
  C2E_COLLECTION_TYPE,
  C2E_DATASET_TYPE,
  C2E_CONTENT_TYPE_COLLECTION_ID,
  C2E_SOURCE_CODE_ID,
  C2E_PERSON_TYPE,
  C2E_ORGANIZATION_TYPE,
  C2E_CODE_TYPE,
  C2E_CREATIVE_WORK_TYPE,
} from '../constants';
...
// import { currikiConverter } from "./currikic2eConerter"
...
export default class C2eWriter {
  ...
  c2eResourcesToCreate: Array<{
    sourceFilePath: string;
    c2eResource: C2eDigitalDocument;
  }> = [];
  c2eContentsToCreate: Array<{
    c2eContent: Record<string, any>;
    c2eContentDocument: C2eContentDocumentLd | undefined;
  }> = [];
  ...
  constructor(c2eId: string) {
    this.c2eId = c2eId;
    this.c2eResourceCollectionLd = new C2eResourceCollectionLd(
      C2E_RESOURCE_COLLECTION_ID,
      C2E_COLLECTION_TYPE
    );
    this.c2eContentTypeCollectionLd = new C2eContentTypeCollectionLd(
      C2E_CONTENT_TYPE_COLLECTION_ID,
      C2E_COLLECTION_TYPE
    );
  ...
  createC2e(c2ePath: string = ''): Boolean {
    ...
    if (this.ok) {
      ...
      c2ePackageCreator.createC2ePackage(
        this.getC2e().toJsonLd(),
        this.c2eResourcesToCreate,
        this.getC2eContents(),
        this.c2eContentsToCreate,
        this.getC2e()
      );
    }
    return this.ok;
  ...
  createC2eResource(
    ...
    const c2eResource: C2eDigitalDocumentLd =
      this.c2eResourceCollectionLd.addC2eResource(
        new C2eDigitalDocumentLd(
          this.c2eId,
          C2E_DIGITAL_DOCUMENT_TYPE,
          targetFilePath,
          MIMEType
  ...
  defineC2eContentType(
    contentTypeName: string,
    contentTypeAttributes: Array<{ property: string; type: string }>
  ): void {
    if (contentTypeAttributes.length > 0) {
      const contentTypeAttributeList = contentTypeAttributes.map(
        (contentTypeAttribute) => {
          const attribute = new C2eContentTypeAttributeLd(
            contentTypeAttribute.property,
            contentTypeAttribute.type
          );
          return attribute;
        }
      );
      const contentType = new C2eContentTypeLd(
        this.c2eId,
        contentTypeName,
        contentTypeAttributeList
      );
  ...
  createC2eContent(
  ...
  ): C2eContentDocumentLd | undefined {
    let c2eContentDocument = undefined;
    const identifier =
      'c2ens:c2eid-' + this.c2eId + '/content-type/' + contentTypeName;
    const contentTypeArr = this.c2eContentTypeCollectionLd
      .getC2eContentTypes()
      ?.filter(
        (c2eContentType) => c2eContentType.getIdentifier() === identifier
      );
    const contentType =
      contentTypeArr?.length && contentTypeArr?.length > 0
        ? contentTypeArr[0]
        : null;
    const contentTypeAttributes = contentType
      ?.getAttributes()
    ...
    if (contentType === null) {
      ...
      this.errors.push(
        '"' +
          contentTypeName +
          '" Content Type not found while creating content!'
      );
    }

    if (
      typeof content === 'object' &&
      (contentTypeAttributes ? contentTypeAttributes?.length : 0) > 0 &&
      Object.keys(content).length > 0 &&
      JSON.stringify(Object.keys(content)) ===
        JSON.stringify(contentTypeAttributes)
    ) {
      const firstKey = contentTypeAttributes ? contentTypeAttributes[0] : 0;
      const firstVal = content[firstKey];
      const existingFirstVals = this.c2eContentsToCreate.filter(
        (contentToCreate) => contentToCreate.c2eContent[firstKey] === firstVal
      );
      const fileCounter =
  ...
  createC2eMetadata(metadata: {
    ...
    const c2eMdGeneralLd = new C2eMdGeneralLd(
      this.c2eId,
      C2E_DATASET_TYPE,
      metadata.general.title,
      metadata.general.description,
      metadata.general.keywords ? metadata.general.keywords : []
    );
    const c2eAuthorLd = new C2eAuthorLd(
      this.c2eId,
      C2E_PERSON_TYPE,
      metadata.author.name,
      metadata.author.email,
      metadata.author.url ? metadata.author.url : ''
    );
    const c2ePublisherLd = new C2ePublisherLd(
      this.c2eId,
      C2E_ORGANIZATION_TYPE,
      metadata.publisher?.name ? metadata.publisher?.name : '',
      metadata.publisher?.email ? metadata.publisher?.email : '',
      metadata.publisher?.url ? metadata.publisher?.url : ''
    );
    const c2eMdCopyrightHolderLd = new C2eMdCopyrightHolderLd(
      this.c2eId,
      C2E_PERSON_TYPE,
      metadata.copyrightHolder?.name ? metadata.copyrightHolder?.name : '',
      metadata.copyrightHolder?.email ? metadata.copyrightHolder?.email : '',
      metadata.copyrightHolder?.url ? metadata.copyrightHolder?.url : ''
    );
    const c2eLicenseDigitalDocumentLd = new C2eDigitalDocumentLd(
      this.c2eId,
      C2E_DIGITAL_DOCUMENT_TYPE,
      metadata.license?.file ? metadata.license?.file : '',
      metadata.license?.type ? metadata.license?.type : ''
    );
    const c2eMdCopyrightLd = new C2eMdCopyrightLd(
      this.c2eId,
      C2E_DATASET_TYPE,
      c2eLicenseDigitalDocumentLd,
      c2eMdCopyrightHolderLd,
      metadata?.copyrightNote ? metadata?.copyrightNote : '',
      metadata?.copyrightYear ? metadata?.copyrightYear : ''
    );
    const c2eMdLifecycleLd = new C2eMdLifecycleLd(
      this.c2eId,
      C2E_CODE_TYPE,
      metadata?.codeVersion ? metadata?.codeVersion : 'v1.0',
      metadata?.codeStatus ? metadata?.codeStatus : ''
    );
    ...
    this.c2eMetadata.setSchemaVersion(
      metadata?.version ? metadata.version : 'v1.0'
    );
  }

  getReaderSourceCode(): C2eSourceCodeLd {
    const htmlDocument = new C2eSourceCodeDocumentLd(
      this.c2eId,
      'index.html',
      'text/html'
    );
    const javascriptDocument = new C2eSourceCodeDocumentLd(
      this.c2eId,
      'index.js',
      'application/javascript'
    );
    const c2eSourceCode = new C2eSourceCodeLd(
      C2E_SOURCE_CODE_ID,
      C2E_COLLECTION_TYPE,
      htmlDocument,
      javascriptDocument
  ...
  // c2eExecuteEntryFile() {
  //     currikiConverter('F:\\curriki studio\\zip.zip')
  // }

  c2eExtractZip(filePath: String) {
    return ZipConverter(filePath);
  }

  getContainer() {
    const c2eContainer = new C2eContainerLd(
      this.getResourceCollection(),
      this.getc2eContentTypeCollection(),
      this.getReaderSourceCode(),
      this.getC2eContentCatalog()
    );
    return c2eContainer;
  }
